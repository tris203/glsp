...
import (
	...
	"flag"
	"fmt"
	"github.com/tliron/glsp/tools/shared"
	"go/format"
	"log"
	"os"
	"os/exec"
...
const vscodeRepo = shared.VSCodeRepo
...
var lspGitRef = shared.LSPGitRef
...
func processinline() {
	...
	if *repodir == "" {
		...
		cmd := exec.Command("git", "clone", "--quiet", "--depth=1", "-c", "advice.detachedHead=false", shared.VSCodeRepo, "--branch="+shared.LSPGitRef, "--single-branch", tmpdir)
